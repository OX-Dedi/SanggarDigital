...
<div class="container">
    <div class="header">
        <a href="guru_kelas_saya.php"><button>← Kembali</button></a>
                    ...
                    <article class="materi-item">
                        ...
                        <div class="materi-links">
                            <?php if ($m['file_materi']): ?>
                                📎 <a href="<?= htmlspecialchars($m['file_materi']) ?>" target="_blank">Download File</a><br>
                            <?php endif; ?>
                            <?php if ($m['link_video']): ?>
                                <iframe src="<?= convertYoutubeToEmbed($m['link_video']) ?>" width="100%" height="315" frameborder="0" allowfullscreen></iframe><br>
                            <?php endif; ?>
                            <?php if ($m['link_zoom']): ?>
                                🔗 <a href="<?= $m['link_zoom'] ?>" target="_blank">Link Zoom</a><br>
                            <?php endif; ?>
                            <?php if ($m['link_absen']): ?>
                                📝 <a href="<?= $m['link_absen'] ?>" target="_blank">Link Absen</a><br>
                            <?php endif; ?>
                            <?php if ($m['video_belajar']): ?>
                                <iframe src="<?= convertYoutubeToEmbed($m['video_belajar']) ?>" width="100%" height="315" frameborder="0" allowfullscreen></iframe><br>
                            <?php endif; ?>
                        </div>
                    </article>
                <?php endwhile; ?>
            ...
            <?php if ($user_role === 'guru'): ?>
                ...
                <div id="form-materi" style="display:none;">
                    <form method="POST" enctype="multipart/form-data">
                        ...
                        <textarea name="penjelasan" placeholder="Penjelasan" required></textarea>
                        <input type="file" name="file_materi" />
                        <input type="url" name="link_video" placeholder="Link Video (opsional)" />
                        <input type="url" name="link_zoom" placeholder="Link Zoom (opsional)" />
                        <input type="url" name="link_absen" placeholder="Link Absen (opsional)" />
                        <input type="url" name="video_belajar" placeholder="Video Belajar (opsional)" />
                        <button type="submit" name="submit_materi">Simpan</button>
                    </form>
                </div>
            <?php endif; ?>
        </section>
